...
class IndexController
{
    public function index(Request $request)
    {
        ...
        /* ===== 首页推荐（动态从API获取） ===== */
        $recommendList = $this->getHotRecommendations($info);
     ...
     * 解析红牛资源搜索页面 HTML
     */
    private static function parseHongniuSearchHtml(string $html, string $channelName, int $channelId): array
    {
        $results = [];
     ...
     * 获取首页热播推荐（动态从API获取）
     */
    private function getHotRecommendations(array $info): array
    {
        $channel = $info['channel'] ?? [];
        $vodData = $info['data'] ?? [];
        $classList = $vodData['class'] ?? [];
        ...
        foreach ($categories as [$unifiedTid, $typeName]) {
            // 获取子类ID
            $childTypeIds = \common\VideoUtils::getChildrenTypeIds($unifiedTid, $classList);
            
            if (empty($childTypeIds)) {
                // 没有子类，尝试直接使用大类ID
                $channelTid = \common\VideoUtils::resolveTypeId($unifiedTid, $classList);
                if ($channelTid > 0) {
                    $childTypeIds = [$channelTid];
                }
            }
            
            // 获取该分类的视频
            $vodList = \common\VideoUtils::getVodList($unifiedTid, $childTypeIds);
            
            if ($vodList && !empty($vodList['list'])) {
                $vlist = [];
                $limit = min(6, count($vodList['list']));
                for ($i = 0; $i < $limit; $i++) {
                    $item = $vodList['list'][$i];
                    $vlist[] = [
